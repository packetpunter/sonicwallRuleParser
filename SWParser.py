...
class SonicWallFirewallConfig:

    def init(self):
        self.decoded_data = ""

    def readConfig(self, input_file):
        with open(input_file, 'r') as f:
            read_data = f.readline()
        f.close()
        self.decoded_data = base64.b64decode(read_data)
        self.decoded_data =  self.decoded_data.split(b"&")
    ...
    def parse(self):
        ...
        for line in self.decoded_data:
            ...
            if re.match('^(iface|interface)', line):
                if re.match('^iface_ifnum_', line):
                    ...
                    ifaceName = parse.unquote(ifaceName)
                    ...
                    if interfaceZone:
                        interfaceZone = parse.unquote(interfaceZone)
                    else: 
                    ...
                    if ifaceComment:
                        ifaceComment = parse.unquote(ifaceComment)
                    else:
            ...
            if re.match('^policy', line):
                ...
                if ruleSrcZone and ruleDestZone and ruleSrcNet and ruleDestNet and ruleDestService and ruleAction and ruleStatus and ruleComment:
                    ...
                    rule={
                        ...
                        "ruleSrcNet": parse.unquote(ruleSrcNet),
                        "ruleDestNet": parse.unquote(ruleDestNet),
                        "ruleDestService": parse.unquote(ruleDestService),
                        "ruleAction": ruleAction,
                        "ruleStatus": ruleStatus,
                        "ruleComment": parse.unquote(ruleComment)
                    }
                    rules.append(rule)
                    ruleSrcZone=""
                    ruleDestZone=""
            ...
            if re.match('^addro_', line):
                if re.match('^addro_atomToGrp_', line):
                    ...
                    groupObject = parse.unquote(groupObject)
                    ...
                    groupName = parse.unquote(groupName)
            ...
            if re.match('^addrObjFqdn', line):
                if re.match('^addrObjFqdnId_', line):
                    ...
                    addrName = parse.unquote(addrName)
            ...
            if re.match('^so_', line):
                if re.match('^so_atomToGrp_', line):
                    ...
                    sgroupObject = parse.unquote(sgroupObject)
                    ...
                    sgroupName = parse.unquote(sgroupName)
            ...
            if re.match('^svcObj', line):
                if re.match('^svcObjId_', line):
                    ...
                    serviceName = parse.unquote(serviceName)
            ...
            if re.match('^natPolicy', line):
                ...
                if natRuleID and natOrigSrc and natOrigDest and natOrigService and natTransSrc and natTransDest and natTransService and natSrcInterface and natDestInterface and natReflexive and natComment and natStatus:
                    ...
                    natRule= {
                        "natRuleID": natRuleID,
                        "natOrigSrc": parse.unquote(natOrigSrc),
                        "natOrigDest": parse.unquote(natOrigDest),
                        "natOrigService": parse.unquote(natOrigService),
                        "natTransSrc": parse.unquote(natTransSrc),
                        "natTransDest": parse.unquote(natTransDest),
                        "natTransService": parse.unquote(natTransService),
                        "natSrcInterface": parse.unquote(natSrcInterface),
                        "natDestInterface": parse.unquote(natDestInterface),
                        "natSrcZone": parse.unquote(natSrcZone),
                        "natDestZone": parse.unquote(natDestZone),
                        "natReflexive": parse.unquote(natReflexive),
                        "natComment": parse.unquote(natComment),
                        "natStatus": natStatus,
                    }
        ...
        # print ("")
        self.oInterfaceHdr = "ifaceIfNum, ifaceName, ifaceType, interfaceZone, ifaceIp, ifaceMask, ifaceVlanTag, ifaceVlanParent, ifaceComment"
        self.oInterfaces = collections.OrderedDict(sorted(interfaces.items()))
        ...
        self.oRulesHdr = "RuleID,Source Zone,Dest Zone,Source Net,Dest Net, Dest Service, Action, Status, Comment"
        for x in rules:
        ...
        # print ("================== Nat Rules ========================\n")
        self.oNatRuleHdr = "natRuleID, natOrigSrc,  natTransSrc, natOrigService, natOrigDest, natTransDest, natTransService, natSrcInterface, natSrcZone, natDestInterface, natDestzone, natReflexive, natStatus, natComment"
        for x in natRules:
            ...
            self.oNatRules.append(x)

        # print ("\n================== IP Address Objects ====================\n")
        self.oAddrObjectsHdr = "Object Name,Zone,IP,Subnet"
        ...
        self.oAddrFqdnObjectsHdr = "Object Name,Zone,FQDN"
        self.oAddrFqdnObjects = collections.OrderedDict(sorted(addrFqdnObjects.items()))
        ...
        #         print ("\t%s" % groupObj)
        #     print("\n")
        self.oAddrGroups = addrGroups.items()

        # print ("\n================== Service Objects =======================\n")
        self.oServiceObjectsHdr = "Service Name, Start Port, EndPort, Protocol, ObjectType"
